...
func TestService_Create(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(s *StorageMock) {
				s.On("Save", mock.AnythingOfType("backgroundCtx"), receivedBook).Return(Book{}, assert.AnError)
			},
...
func TestService_GetById(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func(s *StorageMock) {
				s.On("GetById", mock.AnythingOfType("backgroundCtx"), "a-random-book-id").Return(Book{}, assert.AnError)
			},
			wantErr: assert.AnError,
		},
		{
